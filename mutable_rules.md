...
**307** There exists a map called the 'gameboard.' An up to date version of the 'gameboard' shall be kept with the rest of the Wumpusland documentation. The 'gameboard' contains at least two completely distinct areas; one labeled "Start," and one labeled "Finnish." When any player lands on the 'Finnish' area, no further play is possible. That player gets another turn. No player may move on the 'gameboard' until the rules of such movement are further clarified in future rules.  
*Proposed by Richie on 1/17/12, last amended by Craig (317) on 2/15/12*
...
*Proposed by Shawn on 2/6/12, last amended by N/A on N/A*

**316** We hereby define the time at the start of Round 1 as "0 AW" (After Wump). At the end of each round, the current time increments by one 'wumptime', also referred to as a 'year'. This time increments whether or not a proposal is actually given or if the round is skipped. So for example, Pete's turn was time "15 AW" and after his round was skipped the time currently is "16 AW." Time before the current time is denoted "T BW" (Before Wump), where here 'T' is the number of wumptimes or 'years' before the beginning of Round 1.   
The past of the game and thus the gameboard is defined as follows:  
The wumpusland universe and gameboard was created 6000 'years' before the start of the game (6000 BW). The finnish area existed from the beginning of wumpus time. The start area and the wumpus warehouse were created in 0 AW. Once an object is created in wumpus time, it exists from that point up indefinitely into the future until another object or event changes or removes it. So for now the start area exists for any time T >= 0 AW. If an action occurs at time T, the effect immediately takes place in all times T' > T. So if a gamepiece is at time T' in the future, and the current proposal at time T < T' states "A fireball destroys the wumpus warehouse", the warehouse will immediately also be destroyed at time T' and that gamepiece in the future may no longer use it. When any new object or action is defined, if it's time of creation is not specified, it by default is assumed to be the current year.  
The guiding principle should be that you consider mostly only the effects on the gameboard and its associated objects, not the gamepiece or its associated objects, and do not propagate changes to "solve" paradoxes. In addition, gamepieces that are currently moved into the future or the past should usually not be effected. So for example, if I travel to the future and in 15 AW buy a hammer, but in 10 AW someone destroys the warehouse, my hammer still exists, but I shouldn't expect any other inhabitants of the future gameboard that I encounter to have purchased hammers since 10 AW.  
Note that these principles can be applied to things other than physical objects: if you replace "hammer" with "ninja skills" and "warehouse" with "ninja master" in the above example, it will still hold.  
On the other hand if I travel to 15 AW and build a castle at a point, and in 10 AW that point is turned into a swamp, I can expect my future castle to burn, fall over, and sink into the swamp.  
*Proposed by Richie on 2/12/12, last amended by Craig (317) on 2/15/12*

**317** The Finish area on the game board, and all references to it in any pre-existing rules, are replaced by the term, "Finnish area." No player may enter the Finnish area unless they have a full set of winter gear (coat, wool knit cap, and snow shoes. pants optional), are holding a plate of lox (of any kind, including wumplox, ultralox, or negalox), and have been granted permission by a Wumpmoose.  
*Proposed by Craig on 2/15/12, last amended by N/A on N/A*
